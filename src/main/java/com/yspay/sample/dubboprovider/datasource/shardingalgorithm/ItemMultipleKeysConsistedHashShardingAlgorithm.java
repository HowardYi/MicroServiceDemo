...
public abstract class ItemMultipleKeysConsistedHashShardingAlgorithm
	...
	public Collection<String> doSharding(
		...
		String itemIdColumnName = "item_id";
		String itemIdValue = "";
		...
		for (ShardingValue<?> shardingValue : shardingValues) {
			...
			if (shardingValue.getColumnName()
					.equalsIgnoreCase(itemIdColumnName)) {
				itemIdValue = (String) shardingValue.getValue();
			}
		}

		if ((userIdValue.trim().equals("") || orderIdValue.trim().equals(""))
				&& (orderSeqIdValue.trim().equals(""))
				&& (itemIdValue.trim().equals(""))) {
			throw new RuntimeException("数据库路由失败，没有设置路由字段值!");
		}

		ShardInfo shardInfo = null;
		if (!itemIdValue.trim().equals("")) {
			shardInfo = router.getDatasourceTable(
					Long.parseLong(itemIdValue.substring(16), 16));
		}
